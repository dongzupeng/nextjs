...
export default function LikesPage() {
  ...
  const loadLikedPosts = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/users/likes?page=${pagination.page}&limit=${pagination.limit}`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {posts.length === 0 ? (
        <div className="text-center py-12">
          <h2 className="mb-4 text-xl font-medium">还没有点赞的文章</h2>
          <p className="mb-6 text-muted-foreground">去浏览文章并点赞吧！</p>
          <Link
            href="/blog"
